const prefixes =  "prefixes": [
        ...
        {
            "ipv6Prefix": "2001:4860:4801:7e::/64"
        },
        {
            "ipv6Prefix": "2001:4860:4801:7f::/64"
        },
        {
        ...
        {
            "ipv4Prefix": "66.249.78.192/27"
        },
        {
            "ipv4Prefix": "66.249.78.224/27"
        },
        {
...
function isIPv6InRange(ip, cidr) {
  ...
  const prefixParts = Math.floor(bits / 16);
  const ipParts = ip.split(':');
  const prefixIpParts = prefix.split(':');
  for (let i = 0; i < prefixParts; i++) {
    if (ipParts[i] !== prefixIpParts[i]) return false;
  }
  return true;
}
...
export default {
  async fetch(request, env) {
    ...
    const isAllowed = prefixes.some(entry => {
      if (isV6 && entry.ipv6Prefix) return isIPv6InRange(ip, entry.ipv6Prefix);
      if (!isV6 && entry.ipv4Prefix) return ipInCIDR(ip, entry.ipv4Prefix);
      return false;
    });
   const withinHours = isWithinBusinessHours();
    
    if (!withinHours || isAllowed) {
      url.pathname = '/index.html';
    } else {
      url.pathname = '/version-b.html';
    }
